...
class Game:
 def __init__(self):
     ...
     #variavel booleana pra mostrar que o jogo ta rodando

     self.esta_rodando = True
     self.fonte = pygame.font.macth_font(constantes.FONTE)
...
def carregar_arquivos(self):
   #carrega os arquivos de áudio e imgs
   diretorio_imagens = os.path.join(os.getcwd(),"imagens")
   self.diretorio_auidos=os.path.join(os.getcwd(), "audio")
   self.spritesheet = os.path.join(diretorio_imagens, constantes.SPRITESHEET)
   self.pacman_start_logo =os.path.join(diretorio_imgens, constantes.PACMAN_START_LOGO)
   self.pacman_start_logo =pygame.image.logo(self.pacman_start.logo).convert()
...
def mostrar_tela_start(self):
   self.mostrar_star_logo(constantes.LARGURA/2, 20)
   self.mostrar_texto(
